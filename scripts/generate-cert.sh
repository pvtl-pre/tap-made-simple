...
echo "DNS.1 = *.$LEARNING_CENTER_INGRESS_DOMAIN" >>$CERT_PATH/req.cnf
echo "DNS.2 = *.$ITERATE_CLUSTER_INGRESS_DOMAIN" >>$CERT_PATH/req.cnf

for ((i = 0; i < $RUN_CLUSTER_COUNT; i++)); do
  RUN_CLUSTER_INGRESS_DOMAIN=$(yq e .clusters.run_clusters[$i].ingress_domain $PARAMS_YAML)

  echo "DNS.$((i + 3)) = *.$RUN_CLUSTER_INGRESS_DOMAIN" >>$CERT_PATH/req.cnf
done
